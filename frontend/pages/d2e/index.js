...
export default function ExploreTasks() {
    ...
    return (
        <div className="bg-d2e-bg-dark text-white font-display min-h-screen">
            <Head>
                <title>Explore Tasks - D2E Marketplace</title>
            </Head>

            {/* Sticky Top Bar */}
            <header className="sticky top-0 z-50 bg-d2e-bg-dark/80 backdrop-blur-md border-b border-white/10">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4 px-4 py-6">
                    <div className="flex-1">
                        <label className="flex flex-col w-full">
                            <div className="flex w-full items-stretch rounded-xl h-12 bg-d2e-surface-dark shadow-sm border border-white/5">
                                <div className="text-d2e-primary/70 flex items-center justify-center pl-4">
                                    <span className="material-symbols-outlined">search</span>
                                </div>
                                <input
                                    className="form-input flex w-full min-w-0 flex-1 border-none bg-transparent focus:outline-0 focus:ring-0 text-base font-normal placeholder:text-[#92c9a4] px-3 transition-colors"
                    ...
                    <div className="flex gap-3 overflow-x-auto no-scrollbar py-1" style={{ scrollbarWidth: 'none' }}>
                        ...
                        <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-full bg-d2e-surface-dark border border-white/5 px-5 text-sm font-semibold">
                            <span>Category</span>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredTasks.map((task) => (
                                <div key={task.id} className="flex flex-col overflow-hidden rounded-2xl border border-white/5 bg-d2e-surface-dark shadow-sm hover:shadow-xl hover:shadow-d2e-primary/5 transition-all duration-300 group">
                                    <div className="relative h-48 w-full overflow-hidden">
                                        ...
                                        <div className="absolute top-4 left-4 bg-black/40 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-d2e-primary border border-d2e-primary/20">
                                    ...
                                    <div className="p-5 flex flex-col flex-1">
                                        ...
                                        <div className="flex items-center gap-2 mb-4">
                                            <div className="size-6 rounded-full bg-d2e-accent-dark flex items-center justify-center overflow-hidden border border-white/10">
                                            ...
                                            <p className="text-[#92c9a4] text-xs font-medium">{task.poster_name || 'Verified Poster'}</p>
                                        </div>

                                        {/* Tags */}
                                        <div className="flex flex-wrap gap-2 mb-6 min-h-[50px]">
                                            {task.required_skills && task.required_skills.slice(0, 3).map((skill, idx) => (
                                                <span key={idx} className="px-2.5 py-1 rounded-md bg-d2e-accent-dark text-slate-300 text-[10px] font-bold uppercase tracking-wider">{skill}</span>
                                            ))}
                                            {task.required_skills?.length > 3 && (
                                                <span className="px-2.5 py-1 rounded-md bg-d2e-accent-dark text-slate-400 text-[10px] font-bold">+{task.required_skills.length - 3}</span>
                                            )}
                                        </div>
                                        ...
                                        <div className="mt-auto pt-5 border-t border-white/5">
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex flex-col gap-1.5 flex-1 pr-4">
                                                    ...
                                                    <div className="w-full h-2 bg-d2e-accent-dark rounded-full overflow-hidden">
                                                        <div
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <span className="material-symbols-outlined text-6xl text-d2e-accent-dark mb-4">search_off</span>
            ...
            {/* Navigation Bar */}
            <nav className="fixed bottom-0 left-0 right-0 z-50 bg-d2e-bg-dark/95 backdrop-blur-xl border-t border-white/10 pb-6 pt-3">
                <div className="flex justify-around items-center max-w-lg mx-auto px-4">
                    <Link href="/d2e/dashboard" className="flex flex-col items-center gap-1.5 text-slate-500 hover:text-d2e-primary transition-colors">
                    ...
                    <div className="relative -top-8">
                        <Link href="/d2e/post-task" className="size-16 bg-d2e-primary text-d2e-bg-dark rounded-2xl shadow-xl shadow-d2e-primary/30 flex items-center justify-center border-4 border-d2e-bg-dark transform hover:rotate-90 transition-transform duration-300">
                            <span className="material-symbols-outlined text-3xl font-black">add</span>
                        </Link>
                    </div>
                    <Link href="/d2e/dashboard?tab=tasks" className="flex flex-col items-center gap-1.5 text-slate-500 hover:text-d2e-primary transition-colors">
                        <span className="material-symbols-outlined">assignment</span>
                        <span className="text-[10px] font-bold uppercase tracking-tight">Tasks</span>
                    </Link>
                    <Link href="/d2e/dashboard?tab=profile" className="flex flex-col items-center gap-1.5 text-slate-500 hover:text-d2e-primary transition-colors">
